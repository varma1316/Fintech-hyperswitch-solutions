...
resource "aws_iam_role" "eks_cluster_role" {
  ...
  description = "IAM role allowing Amazon EKS control plane to manage AWS resources"
...
resource "aws_security_group" "eks_cluster" {
  ...
  description = "Security group for Amazon EKS control plane communication"
...
resource "aws_security_group" "eks_node" {
  ...
  ingress {
    description     = "Allow control plane to pods HTTPS"
    from_port       = 443
    to_port         = 443
    protocol        = "tcp"
    security_groups = [aws_security_group.eks_cluster.id]
